export type MediaEntry = { url: string; order: number; caption?: string | null };
...
export function groupImagesByParent<T extends { url: string; sortOrder: number; caption?: string | null }>(
  rows: T[],
  ...
  for (const row of rows) {
    ...
    const entry: MediaEntry = { url: row.url, order: row.sortOrder, caption: row.caption ?? null };
