...
- Task 2.2 — Admin Shell, Navigation & Responsive Layout: **Next.** Improve the existing Admin layout, navigation, header, and content areas under a consistent, responsive Admin shell. Scope: active-section indication (already working — preserve), Admin navigation layout, **a mobile/tablet navigation replacement for the sidebar that Task 2.1 confirmed — by screenshot, not just CSS — is fully hidden below 820px with no substitute, on every page tested at both 390px and 768px**, Public/Admin navigation separation (already clean — preserve), shared page title and action area, **overflowing table and form fields (confirmed real and screenshotted at 390px on the Portfolio list, the Portfolio create/edit form, and the Map Editor — root causes: `.adm-table`'s fixed per-column width budget doesn't fit 390px once a table has 3+ non-first columns, and `.adm-field-label-row` doesn't wrap its style-control pills; the Map Editor's 6-column Pins table additionally squeezes its Title column unreadably thin even at 768px, where no scrollbar is triggered but the column is still practically unusable)**.
- Task 2.3 — Admin Dashboard & Content Overview: **Planned.** Turn the Admin home page into a functional hub for daily content management. Scope: quick access to existing content types (already working — preserve), content counts (already working — preserve), recently added/updated content **(confirmed missing — only aggregate counts exist today, no per-item recent list)**, missing-required-field warnings **(confirmed missing — none exist today)**, view-site link (already exists via the sidebar's Back to Site button — extend to mobile once Task 2.2 lands), frequently used actions. Excludes analytics, visitor graphs, or Vercel/Neon dashboard integration.
...
- Task 2.2 — Admin Shell, Navigation & Responsive Layout: Next.
