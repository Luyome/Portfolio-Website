...
- Task 2.3 — Admin Dashboard & Content Overview: **Next.** Turn the Admin home page into a functional hub for daily content management. Scope: quick access to existing content types (already working — preserve), content counts (already working — preserve), recently added/updated content **(confirmed missing — only aggregate counts exist today, no per-item recent list)**, missing-required-field warnings **(confirmed missing — none exist today)**, view-site link (already exists via the sidebar's Back to Site button — extend to mobile once Task 2.2 lands), frequently used actions. Excludes analytics, visitor graphs, or Vercel/Neon dashboard integration.
...
- Task 2.3 — Admin Dashboard & Content Overview: Next.
