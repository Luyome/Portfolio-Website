...
import { db } from "@/db";
import { games, heroButtons, homeContentSelections, homeHeroSlides, homeMapPreview, homeShowcase, homeSkills, models3d, portfolioItems, services, sketches, worldbuildingEntries, worldMaps } from "@/db/schema";
...
import HomeMapPreviewPanel from "@/components/admin/HomeMapPreviewPanel";
...
export default async function AdminHomePage() {
  const [settings, buttons, serviceRows, appearance, heroSlideRows, showcaseRows, selections, skillRows, stats, maps, previewRows, portfolio, sketchRows, modelRows, worldRows, gameRows] = await Promise.all([
    ...
    db.select().from(worldMaps).orderBy(asc(worldMaps.sortOrder), asc(worldMaps.id)), db.select().from(homeMapPreview).where(eq(homeMapPreview.id, 1)),
  ...
  return <div>
    ...
    <div id="home-section-2" className="adm-home-anchor"><HomeCurationPanel title="Selected Work" description="Choose and order up to 6 existing works for the public Home Coverflow." limit={6} candidates={contentCandidates} initialKeys={keysFor("featured_work")} action={saveHomeContentSelections}><input type="hidden" name="section" value="featured_work"/></HomeCurationPanel></div>
    ...
    <div id="home-section-5" className="adm-home-anchor"><HomeMapPreviewPanel maps={maps} defaultMapId={preview?.mapId ?? null} defaultVisible={preview?.isVisible ?? false} action={saveHomeMapPreview} /><Link href="/admin/worldbuilding/atlas" className="adm-home-related">Test on the Map preview →</Link></div>
