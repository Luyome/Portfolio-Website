...
export function HomeSkillsPanel({ initial, action }: { initial: { label: string; isVisible: boolean }[]; action: Action }) {
  ...
  return <section className="adm-home-panel" aria-labelledby="home-skills">
    ...
    <form action={formAction} className="adm-home-form">
      ...
      {items.length ? <ol className="adm-home-selection adm-home-skills-list">{items.map((item, index) => <li className="adm-home-skill-row" key={index}>
        <span className="adm-home-order" aria-label={`Position ${index + 1}`}>{index + 1}</span><label className="adm-home-skill"><span className="sr-only">Skill {index + 1}</span><input value={item.label} onChange={(e) => setItems(items.map((row, i) => i === index ? {...row, label: e.target.value} : row))} required /></label>
        <label className="adm-home-check adm-home-visibility"><input type="checkbox" checked={item.isVisible} onChange={(e) => setItems(items.map((row, i) => i === index ? {...row, isVisible: e.target.checked} : row))} /><span>Visible</span></label>
        <div className="adm-home-controls"><button type="button" disabled={index === 0} onClick={() => { const n=[...items]; [n[index-1],n[index]]=[n[index],n[index-1]]; setItems(n); }} aria-label={`Move ${item.label || `skill ${index + 1}`} up`}>↑</button><button type="button" disabled={index === items.length-1} onClick={() => { const n=[...items]; [n[index+1],n[index]]=[n[index],n[index+1]]; setItems(n); }} aria-label={`Move ${item.label || `skill ${index + 1}`} down`}>↓</button><button type="button" className="danger" onClick={() => setItems(items.filter((_,i)=>i!==index))}>Remove</button></div>
